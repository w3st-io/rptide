<template>
	<div class="px-5 py-5">
		<BRow>
			<BCol cols="12" sm="7">
				<h3 class="text-primary">Your Blog Posts</h3>
			</BCol>

			<!-- Create Product Button -->
			<BCol cols="12" sm="5" class="text-right">	
				<RouterLink to="/blog-post/create">
					<BButton
						variant="primary"
						class="w-100 w-sm-auto"
						style="max-width: 561px;"
					>+ Create a Blog Port</BButton>
				</RouterLink>
			</BCol>

			<BCol cols="12">
				<h6 class="my-3 text-right text-light">
					{{ blogPosts.length }} / {{ blogPostsLimit }}
					<span>
						<RouterLink to="/user">
			...
			>
				<RouterLink :to="`/blog-post/read/${p._id}`" class="text-decoration-none">
					<BCard
						text-variant="light"
						bg-variant="dark"
						border-variant="none"
